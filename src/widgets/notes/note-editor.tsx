...
export const NotesEditor = () => {
  ...
  const addBlock = (index: number, type: Block["type"] = "text", parentId?: string, level?: number) => {
    const newBlock: Block = {
      id: uuidv4(),
      type,
      content: "",
      level: level !== undefined ? level : parentId ? (blocks.find((b) => b.id === parentId)?.level || 0) + 1 : 0,
      parentId,
  ...
  const addNestedBlock = (parentId: string) => {
    ...
    if (parentIndex !== -1) {
      return addBlock(parentIndex, parentBlock.type, parentId, (parentBlock.level || 0) + 1)
    }
    return null
  }
  ...
  const handleMarkdownShortcut = (id: string, content: string) => {
    ...
    // Check for checkbox shortcuts
      ...
      newType = "checkbox"
      newContent = content.startsWith("[ ] ") ? content.substring(4) : content.substring(6)
    }
  ...
  const moveBlock = () => {
    ...
    const newParentId = dragOverInfo.position === "inside" ? dragOverInfo.id : blocks[targetIndex].parentId
    const newLevel =
      dragOverInfo.position === "inside" ? (blocks[targetIndex].level || 0) + 1 : blocks[targetIndex].level || 0
    ...
    if (dragOverInfo.position === "after") {
      ...
      if (targetDescendants.length > 0) {
        const lastDescendantIndex = blocks.findIndex((b) => b.id === targetDescendants[targetDescendants.length - 1].id)
        insertIndex = lastDescendantIndex > insertIndex ? lastDescendantIndex : insertIndex
      }
      insertIndex++
    }

    // Insert the dragged block and its descendants at the appropriate position
    newBlocks.splice(insertIndex, 0, updatedDraggedBlock, ...updatedDescendants)
  ...
  const fromMarkdown = useCallback((markdown: string) => {
    ...
    lines.forEach((line) => {
      ...
      // Ordered list
        ...
        const orderNumber = Number.parseInt(line.trim().match(/^(\d+)/)?.[1] || "1", 10)
        newBlocks.push({
          id: uuidv4(),
          type: "ordered-list",
          content,
      ...
      // Image
      else if (line.trim().startsWith("![") && line.trim().includes("](") && line.trim().endsWith(")")) {
    ...
    <div className="w-full mx-auto p-4 max-w-3xl">
      <div className="flex justify-end gap-2 mb-4">
        ...
        <label>
          ...
          <input type="file" accept=".md" className="hidden" onChange={importMarkdown} />
        </label>
      </div>
      ...
      <div className="space-y-0.5">
        {blocks.map((block, index) => (
          <NoteBlock
            ...
            isDraggedOver={dragOverInfo && dragOverInfo.id === block.id ? dragOverInfo : null}
          />
        ))}
      </div>
      ...
      <div className="flex justify-end mt-4">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "text")}>
              <Text className="mr-2 h-3.5 w-3.5" />
              <span>Текст</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "heading1")}>
              <Heading1 className="mr-2 h-3.5 w-3.5" />
              <span>Заголовок 1</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "heading2")}>
              <Heading2 className="mr-2 h-3.5 w-3.5" />
              <span>Заголовок 2</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "heading3")}>
              <Heading3 className="mr-2 h-3.5 w-3.5" />
              <span>Заголовок 3</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "list")}>
              <List className="mr-2 h-3.5 w-3.5" />
              <span>Список</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "ordered-list")}>
              <ListOrdered className="mr-2 h-3.5 w-3.5" />
              <span>Нумерованный список</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "checkbox")}>
              <CheckSquare className="mr-2 h-3.5 w-3.5" />
              <span>Чекбокс</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "code")}>
              <Code className="mr-2 h-3.5 w-3.5" />
              <span>Блок кода</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "inline-code")}>
              <CodeSquare className="mr-2 h-3.5 w-3.5" />
              <span>Строчный код</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "link")}>
              <Link className="mr-2 h-3.5 w-3.5" />
              <span>Ссылка</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "blockquote")}>
            ...
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "image")}>
              <ImageIcon className="mr-2 h-3.5 w-3.5" />
              <span>Изображение</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => addBlock(blocks.length - 1, "strikethrough")}>
